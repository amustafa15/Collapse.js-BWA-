...
text/x-generic usersearch.php ( PHP script text )
...
$keywords = array_map('trim', explode(',', $_POST['searchKeyword']));

$or = array();

foreach ($keywords as $keyword) {

	$or[] = 'meta_key = "wpum_specialty" AND LOWER(meta_value) = "%'.mysqli_escape_string(strtolower(trim($keyword))).'%"'; 

	$or[] = 'meta_key = "wpum_software" AND LOWER(meta_value) LIKE "%'.mysqli_escape_string(strtolower(trim($keyword))).'%"';

	$or[] = 'meta_key = "wpum_operatingsystem" AND LOWER(meta_value) LIKE "%'.mysqli_escape_string(strtolower(trim($keyword))).'%"';
	
	$or[] = 'meta_key = "first_name" AND LOWER(meta_value) LIKE "%'.mysqli_escape_string(strtolower(trim($keyword))).'%"';

	$or[] = 'meta_key = "last_name" AND LOWER(meta_value) LIKE "%'.mysqli_escape_string(strtolower(trim($keyword))).'%"';

	$or[] = 'meta_key = "description" AND LOWER(meta_value) LIKE "%'.mysqli_escape_string(strtolower(trim($keyword))).'%"';

	$or[] = 'meta_key = "wpum_summary" AND LOWER(meta_value) LIKE "%'.mysqli_escape_string(strtolower(trim($keyword))).'%"';

	$or[] = 'meta_key = "wpum_writer1" AND LOWER(meta_value) LIKE "%'.mysqli_escape_string(strtolower(trim($keyword))).'%"';
...
if (isset($_POST['city']) && !empty($_POST['city'])) {
	...
	$and = 'meta_key = "wpum_City" AND (meta_value="'.

			mysqli_real_escape_string(ucwords(strtolower(trim($_POST['city'])))).'" || meta_value="'.

			mysqli_real_escape_string(strtolower(trim($_POST['city']))).'" || meta_value="'.

			mysqli_real_escape_string(strtoupper(trim($_POST['city']))).'")';
